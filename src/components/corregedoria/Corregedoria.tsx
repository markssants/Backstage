...
import { Plus, Gavel, Download, Trash2, Loader2, Link as LinkIcon, ShieldCheck, Pencil, FileCheck, Receipt, Lightbulb, CheckCircle2, Clock, Calendar, FileText, Sparkles } from "lucide-react";
...
export function Corregedoria({ event, profile }: { event: EventProject, profile: UserProfile }) {
  const [activeSection, setActiveSection] = useState<'contracts' | 'vault'>('contracts');
  ...
  return (
    <div className="space-y-8 p-3 sm:p-6">
      {/* Top Corregedoria Navigation Tabs */}
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 bg-white/5 border border-white/10 p-2 rounded-2xl sm:rounded-3xl backdrop-blur-xl">
        <div className="flex items-center gap-1 sm:gap-2">
          <button
            onClick={() => setActiveSection('contracts')}
            className={cn(
              "flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4 sm:px-6 py-3 rounded-xl sm:rounded-2xl text-xs sm:text-sm font-black uppercase tracking-wider transition-all",
        ...
        {activeSection === 'vault' && (
          <div className="flex items-center gap-2 px-2">
            <Button
              size="sm"
              onClick={() => setActiveSection('contracts')}
              className="w-full sm:w-auto bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/30 rounded-xl text-xs font-bold"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 font-sans">
            ...
            {docs.length === 0 && (
              <div className="col-span-full h-56 border-2 border-dashed border-white/5 rounded-[2.5rem] flex flex-col items-center justify-center space-y-4 text-slate-600 bg-white/5">
                ...
                >
                  <Sparkles className="w-3.5 h-3.5 mr-1.5" /> Gerar Contrato com os Dados do Cliente
                </Button>
              </div>
            )}
          </div>
          ...
          <div className="glass-card rounded-[2.5rem] p-8 text-white flex flex-col md:flex-row items-center justify-between gap-8 border-white/5 shadow-2xl relative overflow-hidden group">
            ...
            <div className="space-y-3 text-center md:text-left relative z-10">
              ...
              <p className="text-slate-400 text-sm max-w-lg leading-relaxed font-medium italic">
                Todos os documentos são armazenados em links e registros seguros. O Backstage protege os dados da sua produtora e dos seus clientes.
              </p>
            </div>
