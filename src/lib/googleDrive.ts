...
export async function requestDrivePermission(): Promise<string | null> {
  try {
    const provider = new GoogleAuthProvider();
    provider.addScope('https://www.googleapis.com/auth/drive.file');
    
    // Sugere a conta ativa logada se houver
    ...
    toast.info("Por favor, selecione sua conta e autorize a permissão do Google Drive no popup que abriu.");
    ...
    if (err.code === 'auth/popup-closed-by-user' || err.code === 'auth/cancelled-popup-request') {
      console.warn("Popup de autorização fechado pelo usuário.");
    } else {
      console.error("Erro ao solicitar permissão de Drive via popup:", err);
    }
    throw err;
  }
  return null;
...
export async function getOrRequestDriveToken(): Promise<string> {
  if (cachedDriveToken) {
    return cachedDriveToken;
  }

  // Abre o modal de popup para obter permissão
  const token = await requestDrivePermission();
  if (!token) {
    throw new Error("Não foi possível obter a permissão de acesso ao Google Drive.");
  }
  return token;
}
